# EduPath 👥

> **Status do Projeto:** 🚀 MVP em Desenvolvimento / Planeamento de Integrações e IA  
> **Product Owner:** [Leandro Gasparello](https://linkedin.com/in/leandrogasparello)

O **EduPath** é uma plataforma de EdTech focada em aprendizagem personalizada e gestão de indicadores de treinamento para empresas e instituições de ensino. O produto centraliza a criação de trilhas de conhecimento, o acompanhamento de progresso e a análise de engajamento em tempo real.

---

## 🗺️ Guia de Navegação do Portfólio

Clique nos links abaixo para explorar a documentação oficial de gestão de produto do EduPath, organizada por etapas do ciclo de vida de desenvolvimento:

### 🎯 [01. Product Discovery](./01-product-discovery/)
* [Product Vision & Proposta de Valor](./01-product-discovery/product-vision.md) — Alinhamento estratégico e pilares do produto.
* [Elevator Pitch](./01-product-discovery/elevator-pitch.md) — Matriz de posicionamento de mercado.
* [Personas do Produto](./01-product-discovery/personas.md) — Perfis detalhados dos utilizadores (Ricardo, Ana e Carlos).
* [Jornada do Utilizador](./01-product-discovery/user-journey.md) — Mapeamento da experiência ponta a ponta.

### 🗂️ [02. Backlog do Produto](./02-backlog/)
* [User Stories & Critérios de Aceite](./02-backlog/user-stories.md) — Backlog detalhado e refinado para engenharia.
* [Definition of Ready (DoR)](./02-backlog/definition-of-ready.md) — Acordo de qualidade para entrada em desenvolvimento.
* [Definition of Done (DoD)](./02-backlog/definition-of-done.md) — Critérios de validação e homologação para subida a produção.

### 🗺️ [03. Planeamento Estratégico](./03-roadmap/)
* [Product Roadmap (18 Meses)](./03-roadmap/product-roadmap.md) — Evolução planeada em 4 fases incrementais.
* [Cronograma de Sprints](./03-roadmap/sprint-planning.md) — Divisão e esforço das Sprints do MVP.

### 📦 [04. Entregas & Release Notes](./04-release-notes/)
* [Release Notes — Sprint 1](./04-release-notes/sprint-01.md) — Autenticação, LGPD e rotas base homologadas.

### 🔄 [05. Evidências Técnicas & Integrações](./05-evidence/)
* [EduPath Sync & Arquitetura](./05-evidence/edupath-sync.md) — Mapeamento de Mensageria, Webhooks, payloads JSON, fluxos do n8n e IA.

---

## 🛠️ Ferramentas Utilizadas no Projeto
* **Gestão de Backlog:** ClickUp / Jira Software
* **Documentação de Produto:** GitHub Wiki / Markdown Docs
* **Framework Ágil:** Scrum / Agile Methodology
* **Automações de Processo:** n8n Workflow Automation
* **Inteligência Artificial:** Agentes de IA para análise de engajamento
